...
void select_good_connection_sigma( TString ifname = "ninja_shifter_tracker_matching_4sigma.sig.root" )
{
	...
	Double_t cut_chi_square = 32. ;
	Double_t cut_lnk_chis = 8. ;
	cout << "*  Select Good Connection\n  cut_chi_square : "<< setprecision(1) << cut_chi_square << endl ; 
	TString psname = Form("Plot_matching_chi_square_cut%.0lf4sigma.pdf", cut_lnk_chis ) ;
...
*/
		...
		//printf("%lf\n",chis_dx + chis_dy + chis_dax + chis_day);
...
{
	SetStyle() ;
	TString psname = "Plot_signal_dp_detail4sigma.pdf" ;
	cout << "\n* Create Canvas \n  FileName : " << psname << endl ;
...
{
	SetStyle() ;
	TString psname = "Plot_signal_da_detail4sigma.pdf" ;
	cout << "\n* Create Canvas \n  FileName : " << psname << endl ;
	TCanvas *c1 = new TCanvas("c1","c1",800,800) ;
	c1->cd() ;
	c1->Print( psname + "[" , "pdf" ) ;
