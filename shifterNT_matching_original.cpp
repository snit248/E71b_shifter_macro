...
public:
	TimeOfSpot()
	{
		time_start = 1572836400 ;  // Nov.4th/2019 12:00:00 (JST)
		time_2hour = 2 * 60 * 60 ;
		time_shift = -65.0 / 24 ;  // shifter time is 65sec. every 4 days. ( -65sec. / 24 spots )
		restart_spotA = 30 ;  // Shifter reset has done on Dec.2nd/2019 12:00:00 because of the trouble on Nov.28th.
    ...
    {
        return time_start + ( ( spotA - 1 ) * 24 + ( spotB - 1 ) ) * ( time_2hour ) ;
    }
    ...
    {
        return GetTimeMin( spotA, spotB ) + time_2hour + time_buf * 2 ;
    }
};
...
public:
	Cut_NTBMSummary()
	{
		...
		widthY = 1200. ;
		widthX = 1200. ;
		time_min = 1572577200 ;  // Nov.1st/2019 12:00:00
		time_max = time_min + ( 60 * 60 * 24 * 120 ) ;  // 120 days
	}
...
{
    string shifter_file_name = "bt061_spot.root";
    string tracker_file_name = "../../../TrackerData/track_b2physics_wagasci_babymind_2021_ninja_nodead_ninjamatch.root";
    string ofname = "ninja_shifter_tracker_matching.root";
...
PartitionOfNTBMSummary InputNTBMSummary( string ifname, Int_t ecc )
{
    ...
    TimeOfSpot time;
    PartitionOfNTBMSummary part_ntbm(27, 25);
