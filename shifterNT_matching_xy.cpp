...
public:
	...
	Shift_dxdy( Int_t ecc )
	{
		...
		else if ( ecc == 6 ) { dx =  -217.;      dy = 103.; } //tuned
...
void shifterNT_matching_xy( Int_t ecc, string shifter_file_name, string tracker_file_name, string ofname );
...
void shifterNT_matching_xy( Int_t ecc )
{
    ...
    string ofname = "ninja_shifter_tracker_matching_4sigma.root";
    shifterNT_matching_xy( ecc, shifter_file_name, tracker_file_name, ofname );
}
...
vector<TMatchingTrack> Matching( vector<TBaseTrackTrackerWall>& v_bt, PartitionOfNTBMSummary& part_ntbm, Int_t ecc )
{
	...
	Double_t cut_dax = 0.2 ;//4sigma
	Double_t cut_day = 0.116 ;//4sigma
	Double_t cut_dx = 13.2 ;//4sigma
	Double_t cut_dy = 14.8 ;//4sigma
	//Double_t cut_da = 0.05 ;//smaller
	//Double_t cut_da = 0.5 ;//larger
	//Double_t cut_dp = 100. ;//larger
	//Double_t cut_dp = 10. ;//smaller
	//Double_t dz_x = 37.0 ;//original
	//Double_t dz_y = 17.0 ;//original
	Double_t dz_x = 0.0 ;
	Double_t dz_y = 0.0 ;
    ...
    for ( auto b = v_bt.begin() ; b != v_bt.end() ; ++b ) {
        ...
        for ( auto n = part_ntbm.GetSpotPartition(b->slow.spot, b->fast.spot).begin() ; n != part_ntbm.GetSpotPartition(b->slow.spot, b->fast.spot).end() ; ++n ) {
            for ( Int_t icluster = 0 ; icluster < n->GetNumberOfNinjaClusters(); ++icluster ) {
				...
				if ( TMath::Abs( n->GetNinjaTangent(icluster, 1) - bt.ax + shift.dax ) > cut_dax ) { continue; }
				if ( TMath::Abs( n->GetNinjaTangent(icluster, 0) - bt.ay + shift.day )  > cut_day ) { continue; }
				if ( TMath::Abs( n->GetNinjaPosition(icluster, 1) - ( bt.x + ( dz_x * bt.ax ) ) + shift.dx ) > cut_dx ) { continue; }
				if ( TMath::Abs( n->GetNinjaPosition(icluster, 0) - ( bt.y + ( dz_y * bt.ay ) ) + shift.dy ) > cut_dy ) { continue; }
                v_match.push_back(
					TMatchingTrack(
						NTBMTrackInfo(
							icluster,
